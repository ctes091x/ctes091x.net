...
読み間違いではありません。まずはスマホにターミナルをインストールします。
...
![Termuxの画面](./termux-display.png)

まずはインストールしたTermux上でSSHコマンドが使用できるようにします。`pkg install openssh`を実行してください。

OpenSSHパッケージのインストールが完了するとSSHコマンドが使えるようになるので、大学の教育用LinuxサーバであるsolにSSH接続します。このとき、コマンドに`-D 1080`というオプションを付加します。
...
![Termuxでsolに接続した様子](./connected-to-sol.png)
...
<p style="font-size:2em;text-align: center;">「「「電通大はAndroidでVPNを使わせろ！！！」」」</p>

ありがとうございました。

来週はraaaさんの「[未定](https://wiki.mma.club.uec.ac.jp)」です。
...
- wacky「[ssh -D と tsocks](https://www.kmc.gr.jp/advent-calendar/ssh/2013/12/14/tsocks.html)」
